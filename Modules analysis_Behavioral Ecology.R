...
# Get 2.5% and 97.5% quantiles for each cell across random permutations
quant_array <- apply(
  randomized_matrices,
  c(1,2),
  quantile,
...
citation("vegan")
# quant_array now has dimensions: 2 × 11 × 11
...
#For this, first we calculated the transitions probability of our observed matrix, dividing each cell number of our observed column by row sum. After that, we isolated the transitions that occurs more than expected by chance #pull out the transitional probability values from the observed matrix
